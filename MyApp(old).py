...
st.info(
    ...
    " More details on the model training in [**this Notebook**](https://)"
)
...
colbis1, colbis2 = st.beta_columns(2)



# Selectors for dependence plot
class_selector = colbis1.selectbox('Genre :', model.classes_)
feature_selector = colbis2.selectbox('Interaction feature :', X_test.columns)

# SHAP dependence plot
dependence_plot, axes = plt.subplots(nrows=5, ncols=3, figsize=(25, 25))

axes = axes.ravel()
dependence_plot.delaxes(axes[14])
dependence_plot.delaxes(axes[13])
...
for index, feature in enumerate(X_test.columns):
    shap.dependence_plot(feature,
                         ...
                         X_test, 
                         interaction_index=X_test.columns.tolist().index(feature_selector), 
                         ax=axes[index], 
                         x_jitter= 0.9, 
                         alpha=0.4, 
                         dot_size=5, 
